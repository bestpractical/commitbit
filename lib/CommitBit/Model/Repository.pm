...
sub write_htpasswd_file {
    ...
    with_write_lock {
        ...
        while (my $user = $users->next) {
            my $line = `htpasswd -nmb @{[($user->nickname||$user->person->email)]} @{[$user->person->__value('password')]}`;
            chomp $line;
            print $fh $line;
        }
    };
...
sub write_svnserve_passwd_file {
    ...
    with_write_lock {
        ...
        my $users = $self->associated_members;
        while (my $user = $users->next) {
            print $fh ($user->nickname||$user->person->email). ' = '. $user->person->__value('password'). "\n";
        }
